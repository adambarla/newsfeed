...
To ensure code quality and maintainability, I refactored shared logic for date parsing and content extraction into the base class. I also enriched the data model to capture `tags` and `image_url` from feeds. Additionally, I implemented HTML cleaning using `BeautifulSoup` to ensure the `content` field contains clean, readable text.

I verified sources include:
- **Ars Technica**
- **Tom's Hardware**
- **Reddit** (r/programming, r/technology, r/cybersecurity, r/artificial)
...
I built a **FastAPI** application that exposes endpoints to retrieve articles by ID and list them with category filtering. I containerized the application using **Docker**.
...
I exposed a new `/api/v1/articles/search` endpoint that enables **semantic search**. This allows users to find relevant articles using natural language queries.

## Scheduler & Observability

I implemented the **Scheduler** using `APScheduler` to automate the ingestion pipeline. It is configured to run every 15 minutes and also triggers an immediate run on application startup.

To ensure the system is production-ready, I:
1.  **Optimized Resources**: Implemented the **Singleton Pattern** (via `@lru_cache`) for heavy dependencies like the Database connection, Embedding Model, and Classifier.
...
4.  **API Performance**: Created a dedicated `ArticleResponse` model to exclude large embedding vectors from the API output.

## Assumptions

1.  **Content Sufficiency**: I assumed that the RSS summaries and Reddit post titles/text provide enough context for the LLM to classify articles accurately without needing to scrape the full external destination page.
2.  **Language**: The system is designed to process English-language content, matching the sources provided.
3.  **Real-time Definition**: I defined "continuous fetching" as a 15-minute polling interval.

## Future Work

I plan to improve the system's robustness and feature set in the following areas:

1.  **Observability**: Transition to a more advanced logging solution (e.g., `structlog`) to enable JSON logging.
...
3.  **Resilience**: Implement robust error handling with retry logic (exponential backoff) for external network calls.
4.  **Scraping**: Integrate a headless browser (like Playwright) to follow Reddit links and scrape the *actual* destination content.
